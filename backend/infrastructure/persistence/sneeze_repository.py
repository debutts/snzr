...
def repo_get_sneeze_by_id(sneeze_id: str):
  with Session(engine) as session:
...
def repo_update_sneeze(id: str, update_request: UpdateSneezeRequest) -> Sneeze:
  with Session(engine) as session:
...
def repo_delete_sneeze(sneeze_id: str) -> None:
  with Session(engine) as session:
    sneeze = session.get(Sneeze, sneeze_id)
    if not sneeze:
      raise ValueError(f"Sneeze with id {sneeze_id} not found")
